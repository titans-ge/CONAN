def mcmc_plots(yval,tarr,farr,earr,xarr,yarr,warr,aarr,sarr,barr,carr, lind, nphot, nRV, indlist, filters,names,RVnames,prefix,params,T0,period):
    ...
    for j in range(nphot):
        ...
        ax[0].set_ylim([np.min(fco[~np.isnan(fco)]) - 0.1 *np.abs((1.0 - np.min(fco[~np.isnan(fco)]))),np.max(fco[~np.isnan(fco)]) + 0.1 * np.abs((np.max(fco[~np.isnan(fco)])-1.0))])

        ax[1].set_ylim([np.min((ft-mt)[~np.isnan(ft-mt)]) - 0.1 * np.min((ft-mt)[~np.isnan(ft-mt)]),np.max((ft-mt)[~np.isnan(ft-mt)]) + 0.1 * np.max((ft-mt)[~np.isnan(ft-mt)])])
    ...
    filters_unique = list(set(filters))



    for filter in filters_unique:
        ...
        ax[0].set_ylim([np.min(flux_phasefold[~np.isnan(flux_phasefold)]) - 0.1 *np.abs((1.0 - np.min(flux_phasefold[~np.isnan(flux_phasefold)]))),np.max(flux_phasefold[~np.isnan(flux_phasefold)]) + 0.1 * np.abs((np.max(flux_phasefold[~np.isnan(flux_phasefold)])-1.0))])

        ax[1].set_ylim([np.min(res_phasefold[~np.isnan(res_phasefold)]) + 0.1 * np.min(res_phasefold[~np.isnan(res_phasefold)]),np.max(res_phasefold[~np.isnan(res_phasefold)]) + 0.1 * np.max(res_phasefold[~np.isnan(res_phasefold)])])
        ...
        fig.savefig(prefix+'_'+str(filter)+'_phasefolded.png')
