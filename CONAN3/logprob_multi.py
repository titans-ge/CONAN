    ...
    *args : list 
    ...
    if fit_sampler == "emcee":
        for jj in range(len(p)):
            this_p = p[jj]*ppm if (pnames[jj].startswith('GPlc') and 'Amp' in pnames[jj]) else p[jj]
            lnprior += prior_distr[jj].logpdf(this_p)  # add the log prior for each jumping parameter
