...
from celerite import solver, terms
from celerite.modeling import ModelSet, ConstantModel

__all__ = ["GPnew"]


class GPnew(ModelSet):
    ...
    def __init__(
        ...
        # Init the superclass
        super(GPnew, self).__init__(models)
    ...
    @property
    def dirty(self):
        return super(GPnew, self).dirty or not self._computed

    @dirty.setter
    def dirty(self, value):
        self._computed = not value
        super(GPnew, self.__class__).dirty.fset(self, value)
    ...
    def log_likelihood(self, y, args, _const=math.log(2.0 * math.pi), quiet=False):
        """
        ...
        resid = y - self.mean.get_value(self._t, args)
        try:
            self._recompute()
    ...
    def predict(self, y, args, t=None, return_cov=True, return_var=False):
        """
        ...
        if t is None:
            ...
            alpha = self.solver.predict(resid, xs) # ML note: NaNs appear here!
        else:
            Kxs = self.get_matrix(xs, self._t)
            alpha = np.dot(Kxs, alpha)

        mu = self.mean.get_value(xs, args) + alpha
    ...
    def sample(self, args, size=None):
        """
        ...
        if size is None:
            return self.mean.get_value(self._t, args) + n[:, 0]
        return self.mean.get_value(self._t, args)[None, :] + n.T

    def sample_conditional(self, y, args, t=None, size=None, regularize=None):
        """
        ...
        """
        mu, cov = self.predict(y, t, args, return_cov=True)
