...
# C Equation 25:
    ...
    def rj(self,x,y,z,p,ERRTOL=0.05,TINY=2.5e-13,BIG=9.e11):
        ...
        assert min(x+y, x+z, y+z, abs(p)) >= TINY, f"rj() in OccultQuadPy: min(x+y, x+z, y+z, abs(p)) < TINY violated (x+y, x+z, y+z, abs(p) = {x+y}, {x+z}, {y+z}, {abs(p)})"

        assert max(x, y, z, abs(p)) <= BIG, f"rj() in OccultQuadPy: max(x, y, z, abs(p)) > BIG violated (x, y, z, abs(p) = {x}, {y}, {z}, {abs(p)})"
