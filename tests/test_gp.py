...
def test_kernels(show_plot=False):
    ...
    #sho
    sp_sho   = spleaf_kernels["sho"](*gp_conv.get_values("sp_sho", data="lc", pars = [amp, len_sc]), 1/np.sqrt(2)).eval(t)
    log_s0, log_w0 =  gp_conv.get_values("ce_sho", data="lc", pars = [amp, len_sc])
    ce_sho   = celerite_kernels["sho"](log_S0=log_s0, log_omega0=log_w0, log_Q=np.log(1/np.sqrt(2))).get_value(t)
